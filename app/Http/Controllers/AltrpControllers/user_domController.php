...
use App\Http\Requests\AltrpRequests\user_domStoreRequest;
use App\Http\Requests\AltrpRequests\user_domUpdateRequest;
// CUSTOM_NAMESPACES_BEGIN - IMPORTANT: Don't remove this comment! Write your namespaces between these comments.

// CUSTOM_NAMESPACES_END - IMPORTANT: Don't remove this comment! Write your namespaces between these comments.

class user_domController extends ApiController
{
    ...
    /**
     * user_domController constructor.
     */
    public function __construct()
    {
        $this->modelClass = user_dom::class;
    }
     ...
     */
    public function store(user_domStoreRequest $request)
    {
        $data = $request->all();
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function update(user_domUpdateRequest $request, $id)
    {
        $data = $request->all();
        if ($fileInfo = $this->hasFileInRequest($request)) {
            $medias = $this->saveMedias($request, $fileInfo['relation']);
    ...
    public function updateColumn(ApiRequest $request, $id, $column)
    {
        ...
        if(! $user) {
            return response()->json(['success'=>false, 'message' =>"user_dom not found"], 404, [], JSON_UNESCAPED_UNICODE);
        }
    ...
    {
        $users = user_dom::selectRaw("$column as value, COUNT($column) as count")
